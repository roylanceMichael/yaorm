...
object ProtobufUtils {
  ...
  private val ProtoNameToProtoTypeMap = HashMap<String, YaormModel.ProtobufType>()
    ...
    descriptor.fields
        .forEach {
          if (ProtoNameToProtoTypeMap.containsKey(it.type.name)) {
            ...
            when {
              it.isRepeated -> // linker table
                return@forEach
              ProtoEnumType == it.type.name -> definition.addColumnDefinitions(
                  buildEnumNameColumnName(it.name, it.number))
              ProtoMessageType == it.type.name -> definition.addColumnDefinitions(
                  buildMessageColumnName(it.name, it.number))
            }
          }
        }
  ...
  fun buildDefinitionGraph(descriptor: Descriptors.Descriptor,
    ...
    val mainDefinition = buildDefinitionFromDescriptor(descriptor,
        customIndexes) ?: return YaormModel.TableDefinitionGraphs.getDefaultInstance()
    val returnGraph = YaormModel.TableDefinitionGraphs.newBuilder().setMainTableDefinition(
        mainDefinition)
        ...
        .forEach {
          if (ProtoEnumType == it.type.name) {
            val definitionGraph = YaormModel.TableDefinitionGraph.newBuilder()
                .setDefinitionGraphType(
                    YaormModel.TableDefinitionGraph.TableDefinitionGraphType.ENUM_TYPE)
                .setMainName(mainDefinition.name)
                .setOtherName(it.enumType.name)
                .setColumnName(it.name)

            val linkerTableDefinition = buildLinkerTableEnum(mainDefinition.name, it.enumType.name,
                it.name)
            ...
            val otherDefinition = buildDefinitionFromDescriptor(it.messageType,
                customIndexes) ?: return@forEach
            val definitionGraph = YaormModel.TableDefinitionGraph.newBuilder()
                .setDefinitionGraphType(
                    YaormModel.TableDefinitionGraph.TableDefinitionGraphType.MESSAGE_TYPE)
                .setMainName(mainDefinition.name)
                .setOtherName(otherDefinition.name)
                .setColumnName(it.name)

            val linkerTableDefinition = buildLinkerTableMessage(mainDefinition.name,
                otherDefinition.name, it.name)
  ...
  fun getIdFromMessage(message: Message): String {
    val foundIdField = message.allFields.keys
        .firstOrNull { it.name == YaormUtils.IdName } ?: Empty
  ...
  fun setIdForMessage(message: Message.Builder, id: String) {
    val foundIdField = message.descriptorForType
        .fields
        .firstOrNull { it.name == YaormUtils.IdName } ?: return
          ...
          if (foundItem == null || foundItem !is List<*> || foundItem.size == 0) {
            ...
            foundItem.filter { it is Message }.forEach {
              ...
              if (foundTableDefinition == null) {
                foundTableDefinition = buildMessageRepeatedRecordTableDefinition(
  ...
  private fun buildMessageColumnName(name: String, order: Int): YaormModel.ColumnDefinition {
  ...
  private fun buildEnumNameColumnName(name: String, order: Int): YaormModel.ColumnDefinition {
    return YaormModel.ColumnDefinition.newBuilder()
        .setName(name)
        .setType(YaormModel.ProtobufType.STRING)
        .setColumnType(YaormModel.ColumnDefinition.ColumnType.ENUM_NAME)
  ...
  private const val MainSuffix = "_main"
  private const val OtherSuffix = "_other"
}
